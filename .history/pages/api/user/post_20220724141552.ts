...
const savePost = async (req: Request, res: Response) => {
  try {
    ...
    if(isFound.length === 0){
        await userModel.findOneAndUpdate({_id:userId}, { $push: { postSaved : postId} as unknown as PushOperator<Document>}  )
        res.status(200).json({
            status: 'Success',
        ...
        await userModel.findOneAndUpdate({_id:userId}, {$pull: { postSaved: req.query.postId} as PullOperator<Document>}, {new: true})
        res.status(200).json({
            status: "Success",
