...
function* handleLogin(payload: LoginPayload) {
    try {
      ...
      yield setCookie('access_token', response.data.access_token, { path: '' })
      yield setCookie('refresh_token', response.data.access_token, { expires: 7, path: '' })
  ...
  function* handleLogout() {
    yield removeCookie('access_token',{ path: '' })
