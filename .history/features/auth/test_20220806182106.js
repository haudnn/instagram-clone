...
const arr2 = shop.map((income) => {
  ...
  listProduct.map((productOrder) => {
    ...
    let total = productOrder.price * product.quantity
      if(productOrder.shopId === income.shopId ) {
        total+= productOrder.price * product.quantity
      }
      console.log( { ...income, income: total })
    }
   })
})
})
