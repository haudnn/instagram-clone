...
const Stories = () => {
  ...
  const handleClick = useCallback((direction: string) => {
    ...
    if (rowRef.current) {
      ...
      rowRef.current.scrollTo({ left: scrollTo, behavior: "smooth" }, 'slow')
    }
  }, [])
  return (
    <section className="h-40 space-y-0.5 md:space-y-2">
      <div className="relative group md:-ml-2">
        <div className="h-6 w-6 absolute top-0 bottom-0 left-2 z-40 m-auto bg-white rounded-full  cursor-pointer flex items-center justify-center">
        ...
        <div className="h-6 w-6 absolute  top-0 bottom-0 right-2 z-40 m-auto bg-white rounded-full  cursor-pointer flex items-center justify-center">
